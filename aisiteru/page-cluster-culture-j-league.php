<?php
/*
Template Name: Cluster Culture - J League
*/
?>
<!DOCTYPE html>
<html lang="ja">
...
<p class="read">
Jリーグは単なるスポーツリーグではなく、地域社会との関係性、クラブ文化、観戦体験、そして日本独自のスポーツ文化と結びついた複合的な構造を持っています。
本クラスタは、構造クラスタ「文化」の下位テーマとして、AI8社の視点から「地域密着」「リーグ制度」「観戦文化」「ビジネス構造」といった論点を構造的に比較した記事のみを収録しています。
正解や優劣を提示するためではなく、Jリーグという文化的現象がどのような構造の上に成立しているのかを読み解くための座標としてご利用ください。
</p>

<p class="cluster-rule">
このクラスタには、構造クラスタ「文化」に属する<strong>Jリーグ</strong>テーマの記事を時系列で表示しています。
</p>
...
// j-league タグ
$season_tag = get_term_by( 'slug', 'j-league', 'post_tag' );
...
<?php endwhile; ?>
<?php else : ?>
<li>「index」タグと「j-league」タグが付いた記事はまだありません。</li>
<?php endif; ?>
