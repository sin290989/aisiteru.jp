<?php
/*
Template Name: Cluster Culture - NPB
*/
?>
<!DOCTYPE html>
<html lang="ja">
...
<p class="read">
日本プロ野球（NPB）は単なるスポーツリーグではなく、ドラフト制度、FA制度、年俸交渉、放映権構造、球団経営、そして地域社会との関係性と結びついた複合的な構造を持っています。
本クラスタは、構造クラスタ「文化」の下位テーマとして、AI8社の視点から「リーグ制度」「市場構造」「観戦文化」「ビジネスモデル」といった論点を構造的に比較した記事のみを収録しています。
正解や優劣を提示するためではなく、日本プロ野球という文化的現象がどのような構造の上に成立しているのかを読み解くための座標としてご利用ください。
</p>

<p class="cluster-rule">
このクラスタには、構造クラスタ「文化」に属する<strong>NPB（日本プロ野球）</strong>テーマの記事を時系列で表示しています。
</p>
...
// npb タグ
$season_tag = get_term_by( 'slug', 'npb', 'post_tag' );
...
<?php endwhile; ?>
<?php else : ?>
<li>「index」タグと「npb」タグが付いた記事はまだありません。</li>
<?php endif; ?>
