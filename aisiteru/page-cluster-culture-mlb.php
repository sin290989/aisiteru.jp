<?php
/*
Template Name: Cluster Culture - MLB
*/
?>
<!DOCTYPE html>
<html lang="ja">
...
<p class="read">
MLB（メジャーリーグベースボール）は単なるスポーツリーグではなく、放映権ビジネス、労使関係、サラリー構造、観戦体験、そしてアメリカ独自のスポーツ文化と結びついた複合的な構造を持っています。
本クラスタは、構造クラスタ「文化」の下位テーマとして、AI8社の視点から「リーグ制度」「市場構造」「観戦文化」「ビジネスモデル」といった論点を構造的に比較した記事のみを収録しています。
正解や優劣を提示するためではなく、MLBという文化的現象がどのような構造の上に成立しているのかを読み解くための座標としてご利用ください。
</p>

<p class="cluster-rule">
このクラスタには、構造クラスタ「文化」に属する<strong>MLB</strong>テーマの記事を時系列で表示しています。
</p>
...
<?php endwhile; ?>
<?php else : ?>
<li>「index」タグと「mlb」タグが付いた記事はまだありません。</li>
<?php endif; ?>
