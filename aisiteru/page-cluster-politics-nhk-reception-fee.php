<?php
/*
Template Name: Cluster Politics - NHK Reception Fee
*/
?>
<!DOCTYPE html>
<html lang="ja">
...
<p class="read">
NHK受信料は、単なる料金制度ではなく、公共放送の成立条件、財源構造、契約制度と結びついた社会制度の一部です。
本クラスタは、構造クラスタ「政治」の下位テーマとして、AI8社の視点から「受信契約の法構造」「税ではない設計理由」「公共性と負担の関係」といった論点を構造的に比較した記事のみを収録しています。
正解や善悪を提示するためではなく、受信料制度が社会構造の中でどのように位置づけられているのかを読み解くための座標としてご利用ください。
</p>

<p class="cluster-rule">
このクラスタには、構造クラスタ「政治」に属する<strong>NHK受信料</strong>テーマの記事を時系列で表示しています。
</p>
...
<?php endwhile; ?>
<?php else : ?>
<li>「index」タグと「nhk-reception-fee」タグが付いた記事はまだありません。</li>
<?php endif; ?>
