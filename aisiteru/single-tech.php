...
?>
<div class="inner">
<a href="/"><span class="home">トップページ</span></a> > <a href="/tech">制作ブログ</a> > <a href="<?php echo $link; ?>"><?php echo $catname; ?></a> > <span class="now"><?php the_title(); ?></span>
</div>
</div>
...
<div class="post-info">
    ...
    <div class="post-cat">
        <span>
        <?php
        // タクソノミーを指定（Tech Notes用に 'tech_category' を使用）
        $taxonomy = 'tech_category'; 
        $terms = get_the_terms(get_the_ID(), $taxonomy);

        if ($terms && !is_wp_error($terms)) {
            foreach ($terms as $term) {
                // 親カテゴリが設定されている場合のみ子カテゴリを表示
                if ($term->parent) {
                    $cat_name = $term->name; // 子カテゴリ名
                    $cat_link = esc_url(get_term_link($term->term_id, $taxonomy)); // 子カテゴリリンク
                    echo sprintf("<a href='%s'>%s</a>", $cat_link, $cat_name);
                }
            }
        } else {
            echo "子カテゴリが設定されていません。";
        }
        ?>
        </span>
        </div>
...
<?php
// カスタムタクソノミー 'tech_tag' のタグを取得
$taxonomy = 'tech_tag'; // タクソノミー名
...
if ($tags && !is_wp_error($tags)): ?>
    <ul class="tag-list">
        <?php foreach ($tags as $tag): ?>
            <li>
                <a href="<?php echo esc_url(get_term_link($tag->term_id, $taxonomy)); ?>">
